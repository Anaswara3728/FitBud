...
// --- SVG Component: Anterior (Front) View [Using Your New SVGs] ---
const AnteriorView = ({ selectedMuscle, onMuscleClick }) => {
    const getClassName = (muscle) =>
        `cursor-pointer transition-colors duration-200 ease-in-out ${selectedMuscle === muscle ? "text-red-500" : "text-gray-400 lg:hover:text-red-300"}`;
    ...
    return (
        ...
        >
            ...
            {/* All <g> elements from both of your provided SVGs are combined here */}
            <g
                id="calves"
                className={getClassName("calves")}
            ...
            <g
                id="abdominals"
                className={getClassName("abdominals")}
                onClick={() => onMuscleClick("abdominals")}
            >
                <path
            ...
            <g
                id="obliques"
                className={getClassName("obliques")}
            ...
            <g
                id="hands"
                className={getClassName("hands")}
                onClick={() => onMuscleClick("hands")}
            >
                <path
            ...
            <g
                id="forearms"
                className={getClassName("forearms")}
            ...
            <g
                id="front-shoulders"
                className={getClassName("front-shoulders")}
                onClick={() => onMuscleClick("front-shoulders")}
            >
                <path
...
const PosteriorView = ({ selectedMuscle, onMuscleClick }) => {
    ...
    return (
        ...
        >
            ...
            <g
                id="hamstrings"
                className={getClassName("hamstrings")}
            ...
            <g
                id="triceps"
                className={getClassName("triceps")}
            ...
            <g
                id="lowerback"
                className={getClassName("lowerback")}
                onClick={() => onMuscleClick("lowerback")}
            >
                <path
            ...
            <g
                id="traps-middle"
                className={getClassName("traps-middle")}
                onClick={() => onMuscleClick("traps-middle")}
            >
                <path
            ...
            <g
                id="rear-shoulders"
                className={getClassName("rear-shoulders")}
                onClick={() => onMuscleClick("rear-shoulders")}
            >
                <path
...
const InteractiveWorkoutPlanner = () => {
    ...
    useEffect(() => {
        try {
            ...
            console.error(
                "Failed to parse workout plans from localStorage",
                error,
            );
            setSavedPlans([]);
        }
    ...
    const handleAddToPlan = (exercise) => {
        ...
        const newExercise = { muscle: selectedMuscle, exercise: exercise };
        if (!currentPlan.some((p) => p.exercise === newExercise.exercise)) {
            setCurrentPlan([...currentPlan, newExercise]);
        }
    };
    ...
    const handleSavePlan = () => {
        if (!planName.trim() || currentPlan.length === 0) {
            alert("Please name your plan and add at least one exercise.");
            return;
        }
        const newPlan = { name: planName, exercises: currentPlan };
        const updatedSavedPlans = [...savedPlans, newPlan];
        setSavedPlans(updatedSavedPlans);
        localStorage.setItem("workoutPlans", JSON.stringify(updatedSavedPlans));
        setCurrentPlan([]);
        setPlanName("");
    };

    const handleDeletePlan = (index) => {
        const updatedSavedPlans = savedPlans.filter((_, i) => i !== index);
        setSavedPlans(updatedSavedPlans);
        localStorage.setItem("workoutPlans", JSON.stringify(updatedSavedPlans));
    };

    const handleLoadPlan = (index) => {
        const planToLoad = savedPlans[index];
        setPlanName(planToLoad.name);
        setCurrentPlan(planToLoad.exercises);
    };
            ...
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-8">
                    ...
                    <p className="text-gray-300 mt-2 text-lg">
                        Build, save, and manage your custom workout routines.
                    </p>
                </header>
                    ...
                    <div className="bg-gray-900 p-4 rounded-lg shadow-lg sticky top-8">
                        <div className="flex justify-center items-center space-x-4 mb-4">
                            <button
                                ...
                                className={`px-6 py-2 rounded-md font-semibold transition-colors ${view === "back" ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}
                            >
                                Back
                            </button>
                            <button
                                onClick={() => setView("front")}
                                className={`px-6 py-2 rounded-md font-semibold transition-colors ${view === "front" ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}
                            >
                                Front
                            </button>
                        </div>
                        ...
                        <div className="bg-gray-900 p-6 rounded-lg shadow-lg">
                            <h2 className="text-2xl font-bold border-b-2 border-red-500 pb-2 mb-4">
                                ...
                                <span className="text-red-400">
                                    {formatMuscleName(selectedMuscle)}
                                </span>
                            </h2>
                            {selectedMuscle && workoutData[selectedMuscle] ? (
                                <ul className="space-y-2">
                                    {workoutData[selectedMuscle].map(
                                        (workout) => (
                                            <li
                                                key={workout}
